...
class MultiTaskPYINClassificationTrainer:
    def __init__(
        self,
        hyperparams,
        model,
    ...
    def train(self):
        ...
        y_true_pinyin = []
        y_pred_pinyin = []
        for inp_mfcc, inp_f0, tone_label, pinyin_label in tqdm(self.train_dataloader):
            inp_mfcc = inp_mfcc.float().to(self.device)
            inp_f0 = inp_f0.float().to(self.device)
            tone_label = tone_label.to(self.device)
            pinyin_label = pinyin_label.to(self.device)
            self.optimizer.zero_grad()
            tone_out, pinyin_out = self.model(inp_mfcc, inp_f0)
            loss = self.criterion(tone_out, tone_label, pinyin_out, pinyin_label)
            loss.backward()
            self.optimizer.step()
    ...
    def validate(self):
        ...
        y_pred_pinyin = []
        with torch.no_grad():
            for inp_mfcc, inp_f0, tone_label, pinyin_label in tqdm(self.valid_dataloader):
                inp_mfcc = inp_mfcc.float().to(self.device)
                inp_f0 = inp_f0.float().to(self.device)
                tone_label = tone_label.to(self.device)
                pinyin_label = pinyin_label.to(self.device)
